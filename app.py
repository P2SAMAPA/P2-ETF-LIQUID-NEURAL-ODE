...
UNIVERSE_COLOURS = {
    ...
    "HYG": "#B7950B", "VNQ": "#6C3483", "GLD": "#CA6F1E", "SLV": "#717D7E",
    "SPY": "#C0392B", "QQQ": "#922B21", "XLK": "#1A5276", "XLF": "#117A65",
...
def load_results() -> pd.DataFrame:
    ...
    try:
        ...
        ds = load_dataset(HF_RESULTS_REPO, split="train",
...
with st.sidebar:
    ...
    ) if False else None   # skip if banner not present
    ...
    lookback = st.slider(
        ...
        min_value=5, max_value=120, value=60, step=5,
    )
...
slow_today   = 1.0 - fast_today - max(0, 1.0 - fast_today - df_today.get("slow_frac", pd.Series([0.0])).mean())
...
with tab1:
    ...
    try:
        ...
        if show_ci and "ci_lower" in df_today.columns:
            fig.add_trace(go.Scatter(
                ...
                y=df_today["ci_upper"],
                mode="markers",
                marker=dict(symbol="line-ew", size=10, color="rgba(100,100,100,0.4)",
    ...
    # Data table
    display_cols = ["rank", "ticker", "score_adj", "score_raw", "ci_lower", "ci_upper", "tau_mean", "fast_frac"]
...
with tab3:
    ...
    try:
        ...
        fig3 = make_subplots(
            ...
            subplot_titles=("Mean τ (days) — Network Tempo", "Fast-Neuron Fraction — Volatility Proxy"),
            vertical_spacing=0.12,
        )

        # τ trace
        fig3.add_trace(go.Scatter(
            x=regime_df["date"], y=regime_df["tau_mean"],
            mode="lines", fill="tozeroy",
            name="τ mean", line=dict(color="#2E86C1", width=1.5),
            fillcolor="rgba(46,134,193,0.15)",
        ), row=1, col=1)
        fig3.add_hline(y=FAST_THRESHOLD, line_dash="dash", line_color="#E74C3C",
                       annotation_text="Fast threshold", row=1, col=1)
        fig3.add_hline(y=SLOW_THRESHOLD, line_dash="dash", line_color="#27AE60",
                       annotation_text="Slow threshold", row=1, col=1)
        ...
        fig3.add_trace(go.Bar(
            x=regime_df["date"], y=regime_df["fast_frac"],
            name="Fast frac", marker_color=fast_colours,
        ), row=2, col=1)
    ...
    if "ci_lower" in df_today.columns and "ci_upper" in df_today.columns:
        ...
        try:
            ...
            fig4.update_layout(
                ...
                xaxis_title="ETF", yaxis_title="Score (z-score)",
                height=420,
                plot_bgcolor="rgba(0,0,0,0)",
                paper_bgcolor="rgba(0,0,0,0)",
            )
            ...
            st.dataframe(df_ci[["ticker", "score_adj", "ci_lower", "ci_upper", "ci_width", "conviction"]])
