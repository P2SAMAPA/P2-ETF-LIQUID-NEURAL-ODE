...
from config import LTCConfig, ModelConfig, TrainingConfig, ODEConfig, DataConfig, ScoringConfig, OutputConfig
...
def synthetic_returns(rng) -> pd.DataFrame:
    ...
    tickers = ["TLT_log_return", "VCIT_log_return", "LQD_log_return",
               "HYG_log_return", "VNQ_log_return", "GLD_log_return", "SLV_log_return"]
...
def mock_config() -> LTCConfig:
    return LTCConfig(
        model=ModelConfig(n_neurons=HIDDEN, n_sensory=4, n_inter=2, n_command=2,
                          input_dim=INPUT_DIM, dropout=0.1,
                          tau_min=0.1, tau_max=5.0, sparsity=0.5),
        training=TrainingConfig(lr=1e-3, epochs=2, patience=5, batch_size=4,
                                warmup_epochs=1, seed=42),
...
def tiny_model() -> LTCModel:
    return LTCModel(
        ...
        tau_min=0.1, tau_max=5.0, dropout=0.1,
...
def batch_tensors(rng):
    B, W, D = 4, WINDOW, INPUT_DIM
    x  = torch.tensor(rng.normal(0, 1, (B, W, D)), dtype=torch.float32)
    dt = torch.ones(B, W, dtype=torch.float32)
    y  = torch.tensor(rng.normal(0, 0.01, (B, N_ETF)), dtype=torch.float32)
    return x, dt, y
