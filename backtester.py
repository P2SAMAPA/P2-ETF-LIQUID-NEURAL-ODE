...
from metrics import sharpe, ic, hit_rate, max_drawdown
...
def run_backtest(
    ...
    log.info("Backtest results: Sharpe=%.3f  MaxDD=%.3f  IC=%.3f  HitRate=%.3f",
             results["sharpe"], results["max_drawdown"], results["ic"], results["hit_rate"])

    # Save outputs
    out = Path(output_dir)
    out.mkdir(parents=True, exist_ok=True)

    eq_df = pd.DataFrame({"date": [str(d.date()) for d in all_dates], "equity": equity, "return": port_r})
