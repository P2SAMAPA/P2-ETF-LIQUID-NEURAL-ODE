...
def run_daily_inference(universe: str = "combined", config_path: str = "ltc_config.toml") -> None:
    ...
    results_df = build_daily_ranking(
        scores=scores_adj, tickers=active_tickers, date=today,
        ci_lower=ci_lower, ci_upper=ci_upper,
        tau_mean=tau_mean, fast_frac=fast_frac, universe=universe,
    )

    log.info("Top 3 ETFs for %s:\n%s", today.date(), results_df[["ticker","score_adj"]].head(3).to_string())
